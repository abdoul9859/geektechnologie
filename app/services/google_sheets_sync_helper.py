...
async def sync_product_stock_to_sheets(product_id: int) -> bool:
    """
    ...
    try:
        ...
        if not spreadsheet_id:
            print("GOOGLE_SHEETS_SPREADSHEET_ID non configure, synchronisation ignoree")
            return False

        # Récupérer le produit
        product = await Product.find_one(Product.product_id == product_id)
        if not product:
            print(f"Produit {product_id} non trouve")
            return False

        # Si le produit n'a pas de code-barres, impossible de le retrouver dans le Google Sheet
        if not product.barcode:
            print(f"Produit {product.name} n'a pas de code-barres, synchronisation impossible")
            return False

        # Initialiser le service Google Sheets
        service = GoogleSheetsService()
        if not service.authenticate():
            print("Echec d'authentification Google Sheets")
            return False
...
async def sync_multiple_products_to_sheets(product_ids: list) -> dict:
    """
    ...
    try:
        ...
        for product_id in product_ids:
            try:
                success = await sync_product_stock_to_sheets(product_id)
                if success:
                    stats['updated'] += 1
                else:
                    stats['skipped'] += 1
